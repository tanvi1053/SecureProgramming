...
class Server:
    ...
    async def send_public_key(self, websocket, message):
        ...
        for server in destination_users:
            if server in self.client_key.keys():
                print(
                    f"Sending to... {self.connected_clients[self.client_key[server]]['websocket']}"
                )
                ...
                await self.connected_clients[self.client_key[sender]]['websocket'].send(json.dumps(key))
            else:
                fail_message = {"type": "user_not_found"}
                await self.connected_clients[self.client_key[sender]]['websocket'].send(json.dumps(fail_message))
    ...
    async def process_signed_data(self, websocket, message):
        if message["data"]["data"]["type"] == "hello":
            ...
            self.connected_clients[f"{client_address[0]}:{client_address[1]}"] = {
                'websocket': websocket,
                'admin': message["data"]["data"]["admin"],
                'username': username
            }

            # Handle user roles
            if message["data"]["data"]["admin"]:
                await self.handle_admin(websocket)
            else:
                await self.handle_member(websocket)
    ...
    async def handle_admin(self, websocket):
        print("Admin is connected to server")
        while True:
            try:
                command = await websocket.recv()
                if command.lower() == 'exit':
                    for member_conn in self.connected_clients.values():
                        if not member_conn['admin']:
                            await member_conn['websocket'].send('exit')
                    break
                elif command.lower() == 'input':
                    # List all available members
                    members = [client['username'] for client in self.connected_clients.values() if not client['admin']]
                    await websocket.send(json.dumps({"type": "member_list", "members": members}))

                    # Admin chooses a member
                    chosen_member = await websocket.recv()
                    if chosen_member in self.client_key:
                        prompt = await websocket.recv()
                        member_conn = self.connected_clients[self.client_key[chosen_member]]['websocket']
                        await member_conn.send('input')
                        await member_conn.send(prompt)
                        user_input = await member_conn.recv()
                        await websocket.send(user_input)
                    else:
                        await websocket.send(json.dumps({"type": "error", "message": "Member not found"}))
                else:
                    for member_conn in self.connected_clients.values():
                        if not member_conn['admin']:
                            await member_conn['websocket'].send(command)
                            output = await member_conn['websocket'].recv()
                            await websocket.send(output)
            except websockets.ConnectionClosed:
                break

    async def handle_member(self, websocket):
        print("Member is connected to server")

    async def broadcast_public_chat(self, websocket, message):
        message_json = json.dumps(message)
        for client_websocket in self.connected_clients.values():
            if client_websocket['websocket'] != websocket:
                await client_websocket['websocket'].send(message_json)
    ...
    async def send_client_update(self):
        ...
        for client in self.connected_clients.values():
            await client['websocket'].send(update_message_json)
    ...
    async def forward_chat(self, message):
        ...
        for server in destination_users:
            if server in self.client_key.keys():
                await self.connected_clients[self.client_key[server]]['websocket'].send(json.dumps(message))
