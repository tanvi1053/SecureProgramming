...
def generate_rsa_keys():
    ...
    public_key = rsa_key.publickey().export_key(format="PEM", pkcs=8)  # Export public key
...
def generate_fingerprint(public_key):
    ...
    public_key = public_key.replace("\n", "").replace("\r", "").replace(" ", "")  # Clean up the public key string
    public_key = "-----BEGIN PUBLIC KEY-----\n" + public_key[26:-24] + "\n-----END PUBLIC KEY-----"  # Reformat the public key string
    sha256_hash = SHA256.new(public_key.encode('utf-8')).digest()  # Create a SHA-256 hash of the public key
    fingerprint = base64.b64encode(sha256_hash).decode('utf-8')  # Encode the hash in Base64
    return fingerprint
...
def decrypt_message(iv, encrypted_aes_key, ciphertext, private_key_pem_file):
    ...
    cipher_rsa = PKCS1_OAEP.new(private_key, hashAlgo=SHA256)  # Create an RSA cipher with the private key
    aes_key = cipher_rsa.decrypt(base64.b64decode(encrypted_aes_key))  # Decrypt the AES key
    cipher = AES.new(aes_key, AES.MODE_GCM, nonce=base64.b64decode(iv))  # Create an AES cipher with the decrypted key and IV
